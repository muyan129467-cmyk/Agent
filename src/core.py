...
class Agent:
    ...
    def handle_tool_call(self, tool_call):
        ...
        try:
            ...
            function_call_content = f"参数解析失败: {str(e)}"
            if self.verbose:
        ...
        for tool in self.tools:
            if getattr(tool, '__name__', None) == function_name:
                tool_func = tool
                break
        
        if tool_func is None:
            raise ValueError(f"未知的工具函数: {function_name}")
        
        # 安全调用工具函数
        try:
            result = tool_func(**function_args)
            function_call_content = str(result) if result is not None else ""
        except Exception as e:
            function_call_content = f"工具调用失败: {str(e)}"
        
        if self.verbose:
            print(f"调用工具: {function_name}, 参数: {function_args}, 结果: {function_call_content[:100]}...")
